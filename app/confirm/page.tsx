...
const items: Array<
  [
    keyof Pick<Order, "lettuce" | "tomato" | "onion" | "cheese" | "beef">,
    string,
    number,
  ]
> = [
...
export default function ConfirmPage() {
  ...
  useEffect(() => {
    ...
    else setOrder(JSON.parse(details) as Order);
  }, [router]);
  ...
  const placeOrder = () => {
    ...
    window.setTimeout(() => {
      ...
      ) as Array<Order & { createdAt: string }>;
    ...
    <>
      ...
      <main className="confirm-page">
        <div className="confirm-intro">
          ...
          <p>Review your order before we fire up the grill.</p>
        </div>
        <div className="receipt-card">
          <div className="receipt-head">
            <span>BURGERIAN</span>
            <span>#{String(Date.now()).slice(-5)}</span>
          </div>
          <h2>Order details</h2>
          ...
          <div className="order-lines">
            {items.map(([id, name, price]) => (
              <div key={id}>
                <span>
                  {name} <small>× {order[id] || 1}</small>
                </span>
                <strong>₹{(order[id] || 1) * price}</strong>
              </div>
            ))}
          </div>
