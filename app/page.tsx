    ...
    <header className="site-header">
      ...
      <nav>
        ...
        <Link href="/orders" className="nav-link">
          Orders <span className="nav-arrow">↗</span>
        </Link>
      </nav>
    </header>
  );
  ...
  return (
    <div className="preview-card">
      ...
      <div className="burger-stage">
        ...
        <div className="sesame sesame-one" />
        <div className="sesame sesame-two" />
        <div className="sesame sesame-three" />
        {ingredients.flatMap(({ id }) =>
          Array.from({ length: counts[id] + 1 }, (_, index) => (
      ...
      <div className="preview-caption">
        <span>
          <strong>
            {Object.values(counts).reduce((sum, value) => sum + value, 0) + 5}
          </strong>{" "}
          layers
        </span>
...
export default function HomePage() {
  ...
  const total = useMemo(
    () =>
      45 +
      ingredients.reduce((sum, item) => sum + counts[item.id] * item.price, 0),
    [counts],
  );
  const updateCount = (id: IngredientId, change: number) =>
    setCounts((current) => ({
      ...current,
      [id]: Math.max(0, current[id] + change),
    }));
  const checkout = () => {
    window.localStorage.setItem("burgerDraft", JSON.stringify(counts));
    window.localStorage.setItem(
      "orderDetails",
      JSON.stringify({ ...counts, totalPrice: total }),
    );
    router.push("/form");
  };
    ...
    <main>
      ...
      {toast && (
        <div className="toast">
          ...
          <div>
            ...
            <span>
              Enjoy your meal! <Link href="/orders">View orders</Link>
            </span>
          </div>
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <p className="hero-description">
            Pick your favourites, stack them high, and get it delivered while
            it&apos;s still hot.
          </p>
          <div className="hero-stats">
            ...
            <span>
              <strong>20 min</strong> average delivery
            </span>
          </div>
        </div>
      ...
      <section className="builder-section">
        <div className="section-heading">
          <div>
            <p className="eyebrow">STEP 01 · CUSTOMIZE</p>
            <h2>Choose your layers</h2>
          </div>
          <p className="base-price">
            Starting at <strong>₹45</strong>
          </p>
        </div>
        ...
        <div className="checkout-bar">
          <div>
            <span className="muted">Your total</span>
            <strong>₹{total}</strong>
          </div>
